...
def compile_and_report_status(mib_files, mib_compiler):
    ...
    """
    requested_bases = {os.path.splitext(f)[0] for f in mib_files} | set(mib_files)
    child_compiled_mibs = []
    ...
    with click.progressbar(mib_files, label="Compiling MIBs: ", show_eta=True, item_show_func=lambda x: x) as pb:
        for mib_file in pb:
            ...
            compiled_mibs = {
                k: v for k, v in mibs_status.items() if str(v) in ('compiled', 'untouched', 'borrowed')
            }

            for mib_name, mib_status in compiled_mibs.items():
                all_compiled_mibs.append(mib_name)
                if (
                    mib_name in requested_bases
                    or os.path.splitext(mib_name)[0] in requested_bases
                    or getattr(mib_status, 'file', None) == mib_file
                ):
...
def generate_trap_db(compiled_mibs, compiled_mibs_sources, no_descr):
    ...
    for compiled_mib_file in compiled_mibs:
        ...
        traps = {
            ...
            if isinstance(v, dict) and v.get('class') == NOTIFICATION_TYPE
        }
...
def get_var_metadata(var_name, mib_name, search_locations=None):
    ...
    return VarMetadata(
        var_entry['oid'], var_entry.get('description', ''), parsed_enum, parsed_bits
    )
